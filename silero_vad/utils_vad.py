...
from utils.utils import ceildiv


def get_speech_probs(audio: torch.Tensor, model, sampling_rate: int = 16000, window_size_samples: int = 512):
    """
    This method is used for splitting long audios into speech chunks using silero VAD
    ...
    probabilities: list of probs
        list containing probabilities of speech presence in each window
    """
...
def prob_to_vad(speech_probs: torch.Tensor, audio_length_samples: int, threshold: float = 0.5,
                sampling_rate: int = 16_000,
                min_speech_duration_ms: int = 250, max_speech_duration_s: float = float('inf'),
                min_silence_duration_ms: int = 100,
                window_size_samples: int = 512, speech_pad_ms: int = 30):

    """
    This function takes the speech probabilities at each time instance and converts it to a VAD
    ...
    if not torch.is_tensor(speech_probs):
        # print(f"Casting speech_probs to Tensor")
        try:
    ...
    audio_length_frames = ceildiv(audio_length_samples, window_size_samples)

    # Convert ms to number of frames
    min_speech_frames = sampling_rate * min_speech_duration_ms / 1000 / window_size_samples
    speech_pad_frames = sampling_rate * speech_pad_ms / 1000 / window_size_samples
    max_speech_frames = (sampling_rate * max_speech_duration_s / window_size_samples) - 1 - 2 * speech_pad_frames
    min_silence_frames = sampling_rate * min_silence_duration_ms / 1000 / window_size_samples
    min_silence_frames_at_max_speech = sampling_rate * 98 / 1000 / window_size_samples
...
def timestamps_to_full_vad(timestamps, audio_len: int, win_size: int):
    vad_sample = torch.zeros((audio_len,))
    vad_frame = torch.zeros((ceildiv(audio_len, win_size),))

    for timestamp in timestamps:
        vad_sample[win_size*timestamp['start']:win_size*timestamp['end']] = 1.
        vad_frame[timestamp['start']:timestamp['end']] = 1.
    return vad_sample, vad_frame


def timestamps2vad(timestamps, audio_len: int, win_size: int):
